...
(function () {
    var ColorSwatches = function (element) {
        ...
        this.labels = this.list.getElementsByClassName(
            "js-color-swatch__label"
        );
    ...
    function initCustomSelect(element) {
        ...
        for (var i = 0; i < element.select.options.length; i++) {
            var ariaChecked =
                    i == element.select.selectedIndex ? "true" : "false",
                customClass =
                    i == element.select.selectedIndex
                        ? " color-swatches__item--selected"
                        : "",
                customAttributes = getSwatchCustomAttr(
        ...
        list.setAttribute(
            "class",
            "color-swatches__list js-color-swatches__list"
        );
    ...
    function handleHoverEvents(element, index) {
        element.swatches[index].addEventListener(
            "mouseenter",
            function (event) {
    ...
    function getSwatchCustomAttr(swatch) {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            customAttr =
                customAttr + attr[0].trim() + '="' + attr[1].trim() + '" ';
        }
        return customAttr;
    }
