                ...
                <div class="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    ...
                    <div class="bg-rifleGreen text-platinum hover:text-white hover:shadow-lg hover:shadow-black/40 transition-all ease-linear image-full">
                        <div class="h-56 w-auto">
                            <a href="{{route('genderCategory.show', [$category->gender, $category])}}"><img class="w-full h-full object-cover object-center" src="{{ Storage::url($category->image) }}" alt="Shoes" /></a>
                        </div>
                        <div class="card-body">
                            <a href="{{route('genderCategory.show', [$category->gender, $category])}}" class="card-title">{{ $category->name }}</a>
                        </div>
                    </div>
                    @endforeach
                </div>
